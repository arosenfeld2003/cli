...
type (
	...
	_ interface {
		~int | MyInt /* ERROR "overlapping terms p.MyInt and ~int" */
	}
	_ interface{ int | any }
	_ interface{ int | ~string | union }
	_ interface {
		int | ~string | interface{ int }
	}
	...
	_ interface {
		~ /* ERROR "invalid use of ~" */ any
	}
	_ interface {
		int | interface /* ERRORx `cannot use .* in union` */ {
			m()
		}
	}
)
...
type (
	...
	_   interface{ foo }
	_   interface {
		~ /* ERROR "invalid use of ~" */ foo
	}
	_ interface {
		~ /* ERROR "invalid use of ~" */ bar
	}
)
...
type (
	...
	_[T interface {
		T /* ERROR "term cannot be a type parameter" */
	}] struct{}
	_[T interface {
		~T /* ERROR "type in term ~T cannot be a type parameter" */
	}] struct{}
	_[T interface {
		int | T /* ERROR "term cannot be a type parameter" */
	}] struct{}
)
...
func _[T interface {
	myInt1 | myInt2
	int
}]() T { return T(0 /* ERROR "cannot convert" */) }
func _[T interface {
	int
	myInt1 | myInt2
}]() T { return T(0 /* ERROR "cannot convert" */) }
...
type (
	...
	Number  interface {
		Integer | Unsigned | Floats | Complex
	}
	Ordered interface {
		Integer | Unsigned | Floats | ~string
	}

	_ interface {
		Number | error /* ERROR "cannot use error in union" */
	}
	_ interface {
		Ordered | comparable /* ERROR "cannot use comparable in union" */
	}
)
